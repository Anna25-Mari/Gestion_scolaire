...
if (isset($_GET['action']) && isset($_GET['id'])) {
    if ($_GET['action'] === 'supprimer') {
        $id = $_GET['id'];
        $pdo->prepare('UPDATE eleves SET classe_id = NULL WHERE classe_id = ?')->execute([$id]);
        $pdo->prepare('DELETE FROM enseignants_classes WHERE classe_id = ?')->execute([$id]);
...
$args = [];
if ($search !== '') {
    $where[] = '(c.nom LIKE ? OR cy.nom LIKE ? OR c.niveau LIKE ?)';
    $like = "%$search%";
...
$stmt = $pdo->prepare("
    SELECT c.id, c.nom, c.niveau, c.capacite, c.description, cy.nom AS cycle_nom
    FROM classes c
    LEFT JOIN cycles cy ON c.cycle_id = cy.id
    $whereSql
    GROUP BY c.id, c.nom, c.niveau, c.capacite, c.description, cy.nom
...
// Effectifs des salles affichées
$effectifs = [];
if (count($classes) > 0) {
    ...
    $stmtEff = $pdo->prepare("
        ...
        FROM eleves
        WHERE classe_id IN ($placeholders)
        GROUP BY classe_id
    ");
    $stmtEff->execute($ids);
    foreach ($stmtEff->fetchAll() as $row) {
        $effectifs[$row['classe_id']] = (int)$row['total'];
    }
}
...
<?php if (isset($_GET['msg'])): ?>
    <?php
    $messages = [
        'ajouter' => 'Salle ajoutée avec succès.',
        'modifier' => 'Salle modifiée avec succès.',
        'supprimer' => 'Salle supprimée avec succès.'
    ];
    ?>
...
<div class="table-card">
    <div class="table-header">
        <h3>Salles par cycle <span class="count-chip"><?= $totalRows ?></span></h3>
        <div style="display:flex;gap:0.5rem;align-items:center;flex-wrap:wrap;">
            <form method="GET" action="classes.php" style="display:flex;gap:0.5rem;align-items:center;flex-wrap:wrap;">
                <input type="text" name="q" placeholder="Rechercher une salle..." class="search-input" style="width:190px;" value="<?= htmlspecialchars($search) ?>">
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <?php if ($classe['cycle_nom'] !== $lastCycle): ?>
            <tr class="cycle-row">
                <td colspan="5"><?= htmlspecialchars($classe['cycle_nom'] ?? 'Sans cycle') ?></td>
            </tr>
            <?php $lastCycle = $classe['cycle_nom']; ?>
            <?php endif; ?>
            <?php $effectif = $effectifs[$classe['id']] ?? 0; ?>
            <tr>
                <td><strong><?= htmlspecialchars($classe['nom']) ?></strong></td>
                <td><?= $classe['niveau'] ? htmlspecialchars($classe['niveau']) : '-' ?></td>
                <td><?= $effectif ?> élève<?= $effectif > 1 ? 's' : '' ?></td>
                <td><?= $classe['capacite'] ?> places</td>
                <td>
                    <div class="actions-cell">
                        <a href="details-classe.php?id=<?= $classe['id'] ?>" class="btn btn-primary btn-sm" title="Voir détails">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                        </a>
                        <a href="modifier-classe.php?id=<?= $classe['id'] ?>" class="btn btn-warning btn-sm" title="Modifier">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                        </a>
                        <a href="?action=supprimer&id=<?= $classe['id'] ?>" class="btn btn-danger btn-sm" title="Supprimer" data-confirm="Supprimer cette salle ? Les élèves ne seront pas supprimés mais deviendront non assignés.">
    ...
    <div class="empty-state">
        ...
        <a href="ajouter-classe.php" class="btn btn-primary btn-sm">Créer une salle</a>
    </div>
    <?php endif; ?>
